...
Definition get_R (s:string) : R := 
  match string_to_R s with
  | None => 0
  | Some s => s
  end. 
...
Compute nth 2  (make_token_list (prog_id) (tokenize "HSF Site
    qubit Q1
    qubit Q2
    qubit Q3 ;
Hamiltonian
...
Definition getNthToken (tl: list token) (index : nat) (t : token) := 
 nth index tl t=t. 


Compute getNthToken (make_token_list (prog_id) (tokenize "HSF Site
    qubit Q1
    qubit Q2
    qubit Q3 ;
Hamiltonian
...
Compute expect {| token_val := id_tok "Q3"; line_number := 0 |}. 

Record HSF_program  := makeHSFProg {
  prog_name: string;
  prog_decls : list string;
...
(*Returns next token in token list*) 

Compute firstExpect  {| token_val := kw_tok qubit_id; line_number := 0 |} parser. 
